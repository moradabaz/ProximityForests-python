...
for d in $CUR_DIR/datasets/*; do
  if [ -d $d ]; then
    ...
    python3 ../application/AppRunner.py $CUR_DIR -name=$dataset -train=$DATASET_DIR/"$dataset"_TRAIN.csv -test=$DATASET_DIR/"$dataset"_TEST.csv -repeat=1 -trees=10 -candidates=2 -targetlast=True -calculate=accuracy
